...
export async function generateMetadata({ params }: { params: Promise<{ business: string }> }) {
  ...
  return demo
    ? { title: `${demo.businessName} | Website Preview`, description: demo.tagline }
    : {};
}
  ...
  return (
    <main className="min-h-screen bg-[#f6f3ed] text-[#171614] pb-24">
      ...
      <section className="mx-auto max-w-7xl px-6 pt-10 lg:px-8 lg:pt-16">
        <div className="overflow-hidden rounded-[2.5rem] bg-[#171614] text-white shadow-2xl">
          <div className="grid min-h-[620px] lg:grid-cols-[1.05fr_.95fr]">
            <div className="flex flex-col justify-between p-8 sm:p-12 lg:p-16">
              ...
              <div className="mt-12 flex flex-wrap gap-3">
                <a href={whatsapp} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-white px-6 py-3.5 text-sm font-black text-black"><MessageCircle size={17} /> WhatsApp</a>
                <a href={`tel:${demo.phone}`} className="inline-flex items-center gap-2 rounded-full border border-white/15 px-6 py-3.5 text-sm font-bold"><Phone size={17} /> Call Now</a>
              </div>
            </div>
            <div className="bg-gradient-to-br from-amber-200 via-orange-100 to-stone-300 p-5 sm:p-8">
              <div className="flex h-full min-h-[520px] flex-col justify-between rounded-[2rem] bg-white/60 p-7 text-black backdrop-blur-sm sm:p-10">
                <div className="flex items-center justify-between"><span className="text-xs font-black uppercase tracking-[0.2em]">{demo.businessName}</span><span className="text-xs font-bold text-black/40">01</span></div>
                <div><p className="text-xs font-black uppercase tracking-[0.2em] text-black/40">Made for local customers</p><p className="mt-4 max-w-md text-4xl font-black tracking-[-0.05em] sm:text-5xl">A website built around your real business.</p></div>
                <div className="grid grid-cols-2 gap-2 text-sm font-bold"><span className="rounded-xl bg-black/5 p-4">Services</span><span className="rounded-xl bg-black/5 p-4">Gallery</span><span className="rounded-xl bg-black/5 p-4">Location</span><span className="rounded-xl bg-black p-4 text-white">Enquire →</span></div>
              </div>
            </div>
          </div>
        </div>
