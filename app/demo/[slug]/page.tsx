...
export default async function DemoPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-white pb-20 sm:pb-0">
      ...
      <div className="fixed inset-x-3 bottom-3 z-50 flex gap-2 rounded-2xl border border-black/10 bg-white/95 p-2 shadow-2xl backdrop-blur sm:hidden"><a href={WHATSAPP} aria-label="Chat on WhatsApp" style={{ backgroundColor: "#25D366", color: "#ffffff" }} className="flex min-h-11 flex-1 items-center justify-center gap-1.5 rounded-xl border border-[#128C7E] px-3 py-2.5 text-xs font-black shadow-sm"><MessageCircle size={15} className="shrink-0" /> <span>WhatsApp</span></a><a href={PHONE} aria-label="Call us" className="flex min-h-11 flex-1 items-center justify-center gap-1.5 rounded-xl border border-white/15 bg-white py-2.5 text-xs font-black !text-black shadow-sm"><Phone size={15} /> <span>Call</span></a></div>
    </main>
  );
}
