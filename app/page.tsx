...
export default function Home() {
  return (
    <main className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_top,#ffffff_0%,#f3f2ee_48%,#e9e8e3_100%)] text-[#11110f]">
      ...
      <section className="mx-auto max-w-7xl px-5 pb-12 pt-9 sm:px-6 sm:pb-16 sm:pt-14 lg:px-8 lg:pb-20 lg:pt-20">
        <div className="max-w-3xl">
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            ...
            <a href={getWhatsAppUrl(contactMessage)} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-12 w-full items-center justify-center rounded-full border border-black/10 bg-white px-5 py-3.5 text-sm font-bold !text-black shadow-sm sm:w-auto">Build my website</a>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-10 sm:px-6 sm:pb-16 lg:px-8"><div className="rounded-[1.5rem] bg-[#11110f] px-5 py-7 text-white shadow-[0_20px_60px_rgba(0,0,0,.12)] sm:rounded-[2rem] sm:px-10 sm:py-12"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/45">Have a business?</p><h2 className="mt-2 text-3xl font-black tracking-[-0.055em] sm:text-5xl">Like a demo? Let&apos;s make it yours.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-white/55">Your branding, photos, services, pricing, location and contact details — designed around your customers.</p><a href={getWhatsAppUrl(contactMessage)} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex min-h-12 w-full items-center justify-center gap-3 rounded-full bg-[#d8ff3e] px-5 py-3.5 text-sm font-black !text-black sm:w-auto">Discuss your website <ArrowRight size={17} aria-hidden="true" /></a></div></section>

      <footer className="mx-auto flex max-w-7xl flex-col gap-3 border-t border-black/10 px-5 py-6 text-xs text-black/40 sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8">
        <div className="flex flex-col gap-1">
          <span className="font-bold text-black/65">{studioConfig.name}</span>
          <span>{studioConfig.email} · {studioConfig.phone}</span>
        </div>
        <span>Local Business Demo Studio · 2026</span>
      </footer>
    </main>
  );
}
