...
const categories = ["All", "Food & Dining", "Beauty", "Health", "Education", "Hospitality", "Retail", "Services"];
...
export default function Home() {
  ...
  const filteredDemos = useMemo(() => {
    ...
    return mapped ? demos.filter((demo) => demo.category === mapped) : demos;
  }, [activeCategory]);
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f6f5f1] text-[#11110f]">
      <nav className="sticky top-0 z-50 mx-auto flex min-h-16 max-w-7xl items-center justify-between border-b border-black/5 bg-[#f6f5f1]/90 px-5 py-4 backdrop-blur-md sm:px-6 lg:px-8" aria-label="Main navigation">
        <a href="/" className="rounded-sm text-sm font-black tracking-[-0.04em] outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-4">{studioConfig.name}</a>
        <div className="hidden items-center gap-6 sm:flex">
          <a href="#demos" className="rounded-sm text-xs font-semibold text-black/50 outline-none transition hover:text-black focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-4">Explore demos</a>
          <a href="#why" className="rounded-sm text-xs font-semibold text-black/50 outline-none transition hover:text-black focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-4">Why it works</a>
        </div>
        <a href={getWhatsAppUrl(contactMessage)} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-10 items-center gap-2 rounded-full bg-[#11110f] px-4 text-xs font-bold text-white shadow-sm outline-none transition hover:-translate-y-0.5 hover:shadow-md focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-4 focus-visible:ring-offset-[#f6f5f1] motion-reduce:transition-none">Talk on WhatsApp <MessageCircle size={14} /></a>
      </nav>

      <section className="mx-auto grid max-w-7xl items-center gap-6 px-5 pb-7 pt-7 sm:gap-8 sm:px-6 sm:pb-16 sm:pt-14 lg:grid-cols-[.9fr_1.1fr] lg:gap-14 lg:px-8 lg:pb-20 lg:pt-20">
        <div>
          <p className="mb-4 text-[10px] font-black uppercase tracking-[0.2em] text-black/35 sm:mb-5">Premium websites for local businesses</p>
          ...
          <p className="mt-5 max-w-xl text-[15px] leading-6 text-black/55 sm:mt-7 sm:text-lg sm:leading-8">Explore premium website concepts for cafés, salons, clinics, stores and more — built around how local customers discover, compare and contact businesses like yours.</p>
          <div className="mt-6 flex flex-col gap-3 sm:mt-7 sm:flex-row">
            <a href="#demos" className="inline-flex min-h-12 items-center justify-center gap-3 rounded-full bg-[#11110f] px-5 py-3.5 text-sm font-black text-white shadow-[0_12px_35px_rgba(0,0,0,.14)] outline-none transition hover:-translate-y-0.5 focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-4 focus-visible:ring-offset-[#f6f5f1] motion-reduce:transition-none">Explore website demos <ArrowRight size={17} /></a>
            <a href={getWhatsAppUrl(contactMessage)} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-12 items-center justify-center rounded-full border border-black/10 bg-white px-5 py-3.5 text-sm font-bold shadow-sm outline-none transition hover:-translate-y-0.5 hover:border-black/20 focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-4 motion-reduce:transition-none">Build my website</a>
          </div>
          <div className="mt-6 grid grid-cols-2 gap-x-5 gap-y-2 border-t border-black/10 pt-5 sm:mt-7 sm:flex sm:flex-wrap sm:gap-x-6">
        ...
        <div className="relative min-h-[255px] sm:min-h-[430px] lg:min-h-[520px]">
          <div className="absolute right-0 top-1 w-[88%] rotate-[1deg] sm:top-3 sm:w-[84%]"><PreviewWindow demo={previewDemos[0]} /></div>
          <div className="absolute bottom-1 left-0 z-10 w-[58%] -rotate-[3deg] sm:bottom-5 sm:w-[52%]"><PreviewWindow demo={previewDemos[1]} /></div>
          <div className="absolute bottom-7 right-0 z-20 w-[47%] rotate-[4deg] sm:bottom-16 sm:w-[42%]"><PreviewWindow demo={previewDemos[2]} /></div>
        </div>
      </section>
      ...
      <section id="demos" className="scroll-mt-20 border-y border-black/10 bg-white">
        <div className="mx-auto max-w-7xl px-5 py-12 sm:px-6 sm:py-16 lg:px-8 lg:py-20">
          <div className="mb-7 flex items-end justify-between gap-4">
            ...
            <span className="hidden shrink-0 rounded-full border border-black/10 bg-[#f6f5f1] px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.12em] text-black/45 sm:inline-flex" aria-live="polite">{filteredDemos.length} concepts</span>
          </div>

          <div className="-mx-1 mb-7 flex gap-2 overflow-x-auto px-1 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" role="tablist" aria-label="Filter demos by business type">
            {categories.map((category) => {
              const selected = activeCategory === category;
              return <button key={category} type="button" role="tab" aria-selected={selected} onClick={() => setActiveCategory(category)} className={`shrink-0 rounded-full border px-4 py-2 text-xs font-bold outline-none transition-all focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-2 ${selected ? "border-[#11110f] bg-[#11110f] text-white shadow-sm" : "border-black/10 bg-[#f6f5f1] text-black/55 hover:border-black/20 hover:text-black"} motion-reduce:transition-none`}>{category}</button>;
            })}
          </div>
